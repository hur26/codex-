...
**验证内容：** 最终正式构建产物基于 `ae16407`，包含锁定圈位保护与 Windows
HTML5 拖放修复。真实升级验收期间曾临时把 Tauri 版本改为 `0.0.9` 构建旧包；
验收后配置已恢复为 `0.1.0`，旧包已删除。升级链路在 `67069f7` 阶段完成验证，
最终产物从 `ae16407` 重新构建。
...
人工验证。累计四次工具指针拖拽均没有触发 drop；随后真人首次拖拽出现禁止
图标且无法放置，由此定位到 Tauri 主窗口原生文件拖放与 HTML5 拖放冲突。设置
`dragDropEnabled: false` 并增加真实配置回归测试后，真人用实体鼠标成功把
左侧 R01 拖到右侧 R02；界面随后确认 R01 空闲、R02 为 Hook/已观测/正在执行
且已选中，因此原生拖放人工验证通过。
...
Task 13 仍不是全部门禁通过：MSI、实体硬件，以及真实 `PermissionRequest`、
真实失败状态和 Codex/驱动重启后的身份连续性等边界仍未完成；已通过的原生
拖放、灯效编辑与 NSIS 安装、卸载、升级不扩展为这些门禁也已通过。
...
版本配置并删除临时旧包后，升级链路证据保持有效。修复 Windows HTML5 拖放
并提交 `ae16407` 后，又重新构建以下正式 `0.1.0` 产物。

| 产物 | 大小（字节） | SHA-256 |
|---|---:|---|
| `D:\Project\codex-halo\apps\desktop\src-tauri\target\release\codex-halo-desktop.exe` | 9,054,208 | `428A6D6C677375A0D47726A95545E945E08544B74A0BE24A51FB49AF2AD6B97F` |
| `D:\Project\codex-halo\apps\desktop\src-tauri\target\release\bundle\nsis\Codex Halo_0.1.0_x64-setup.exe` | 2,176,789 | `DE8E7294A43353EC5CEFDBF4C8E0C3C4F86225922E721E35C92841220C99F528` |
...
- 升级验收后再次静默卸载，注册表项与安装目录均清理；源码配置恢复为
  `0.1.0`，临时 `0.0.9` 安装包已删除。该升级证据来自 `67069f7` 阶段；
- `ae16407` 的最终 NSIS 重新执行静默安装，退出码为 0，注册表版本为 `0.1.0`；
  从安装目录实际启动后显示 `ONLINE / HOOK`。最终再静默卸载，退出码为 0，
  注册表项与安装目录均清理。
...
悬停/指针反馈，没有触发 drop。真人随后首次用实体鼠标复验，拖动时出现禁止
图标，放到圆环后没有反应，确认并非只有自动化工具限制。根因是 Tauri 主窗口
默认启用原生文件拖放，阻止了 Windows WebView 中的 HTML5 拖放；`ae16407`
将主窗口 `dragDropEnabled` 设为 `false`，并新增读取真实 `tauri.conf.json`
的回归测试。目标测试 1/1、全量前端 11 个文件 127/127 与 production build
均通过。修复版 release 中，真人用实体鼠标成功将左侧任务轨 R01 拖到右侧
R02；界面读取随后确认 R01 空闲、R02 为 Hook/已观测/正在执行且已选中。
原生鼠标拖放人工门禁通过。
...
| 鼠标拖拽 | 组件测试与真实 Tauri 配置回归测试通过 | 累计四次工具指针路径未触发 drop；真人首次复验出现禁止图标，定位并修复 Tauri 拖放冲突后，真人成功将任务轨 R01 拖到 R02，界面确认 R01 空闲、R02 为 Hook/已观测/正在执行且已选中 | 通过 |
...
硬件采购继续冻结。下一阶段应在 WiX 工具可用后补齐 MSI；其余尚未通过的真实
Hook 状态与重启连续性门禁也应继续按边界表逐项验证。完成这些软件门禁后，
再决定是否进入硬件选型或签名发布。
