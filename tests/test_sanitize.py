...
class SummarizeEventTests(unittest.TestCase):
    ...
    def test_prioritizes_uppercase_identity_after_large_ordinary_prefix(self):
        payload = {
            f"dynamic-key-{index}": index
            for index in range(10_000)
        }
        payload["THREAD_ID"] = "late-uppercase-thread-secret"
        ...
        self.assertIn("thread_id", summary["top_level_keys"])
        self.assertNotIn("late-uppercase-thread-secret", serialized)
